"""
Escape-time routing vs baselines.
Pre-registro hasheado: ver PREREG_HASH abajo.
Desviaciones declaradas en DEVIATIONS.
"""
import numpy as np
...
PREREG = {
    ...
    "sigma_topic": 3.0, "sigma_cplx": 1.0,   # Régimen B: tema domina
    "R_pole": 1.0, "R_thresh": 10.0, "N_max": 15,
    "eps_list": [0.20, 0.45],                # fallback declarado
    "knn_k": 15,
    "mlp_hidden": 64,                         # se ajusta para param-match
...
DEVIATIONS = [
    "D1: Familia racional = Mobius-cuadratica f(z)=(z^2+c1)/(z+c2); cambiar en rational_map() si la teoria fija otra.",
    "D2: ε calibrado con fallback {0.20, 0.45} (sin descarga PAWS/MRPC en runtime).",
    "D3: kNN reportado con k=15; k=5 disponible con --knn5.",
]
...
def make_data(seed, regime, cfg):
    ...
    n = n_tr + n_te
    # Subespacios ortogonales para tema y complejidad
    Q, _ = np.linalg.qr(rng.standard_normal((d, d)))
    U_topic = Q[:, :K_t]
    U_cplx  = Q[:, K_t:K_t+K_c]
    t_idx = rng.integers(0, K_t, n)
    c_idx = rng.integers(0, K_c, n)
    if regime == "A":   # separable: complejidad domina
        s_t, s_c = 1.0, 3.0
    elif regime == "B": # realista: tema domina (objeción central)
        s_t, s_c = cfg["sigma_topic"], cfg["sigma_cplx"]
    else:               # C: igual a B, paráfrasis se generan en eval
        s_t, s_c = cfg["sigma_topic"], cfg["sigma_cplx"]
    X = s_t * U_topic[:, t_idx].T + s_c * U_cplx[:, c_idx].T + 0.1*rng.standard_normal((n, d))
...
def rational_map(z, c1, c2, R_pole=1.0):
    # Polo regularizado: denominador clampeado
    denom = z + c2
    denom = np.where(np.abs(denom) < 1e-3, 1e-3 + 0j, denom)
    return (z*z + c1) / denom

def escape_features(X, A, N_max=15, R_thresh=10.0):
    # A: (d, 4) real -> c1, c2 complejos; z0 = primera componente compleja de A.T@X
    c = X @ A  # (n, 4)
    c1 = c[:, 0] + 1j*c[:, 1]
    c2 = c[:, 2] + 1j*c[:, 3]
    z = np.zeros_like(c1)
    T = np.full(c1.shape, N_max, dtype=float)
    logz = np.zeros_like(c1, dtype=float)
    ...
    # Lyapunov local: media de log|f'(z_k)|; f' = (2z(z+c2)-(z^2+c1))/(z+c2)^2 — aprox numerica final
...
def fractal_classifier_train(Xtr, ytr, cfg, A_trainable=True, seed=0):
    rng = np.random.default_rng(seed)
    d = Xtr.shape[1]
    A = rng.standard_normal((d, 4)) * 0.1
    K = int(ytr.max()+1)
    if not A_trainable:
        F = escape_features(Xtr, A, cfg["N_max"], cfg["R_thresh"])
        clf = LogisticRegression(max_iter=500).fit(F, ytr)
        return ("frac", A, clf)
    # GD con surrogate hinge ordinal sobre G(x)=log|z_N|
    lr = 0.05
    for step in range(cfg["train_steps_fractal"]):
        F = escape_features(Xtr, A, cfg["N_max"], cfg["R_thresh"])
        G = F[:, 1]  # log|z_N|
        # Hinge ordinal: cada clase tiene su banda; gradiente numerico finito
        eps_fd = 1e-2
...
def build_arms(Xtr, ytr, cfg, seed):
    ...
    # MLP param-matched al fractal (d*4 + clf params ≈ pequeño)
...
def predict(name, model, X, cfg):
    ...
    if name == "4_centroid":
        ...
        return d.argmin(1), -d  # "score" = -dist
...
def metric_stability(name, model, Xte, yte, eps, cfg, seed):
    ...
    same = (y1 == y2)
    correct = (y1 == yte)
    # Estabilidad condicional: misma ruta Y correcta
...
def metric_tescape_auc(name, model, Xte, yte, cfg):
    ...
    T = F[:, 0]
    K = int(yte.max()+1)
    # AUC macro one-vs-rest usando T como score
    aucs = []
    for k in range(K):
        yk = (yte == k).astype(int)
